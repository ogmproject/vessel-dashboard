...
def parse_teluk_lamong(html: str):
    ...
    for line in lines:
        ...
        m = re.match(r"^([A-Z0-9 .\-]+)\s*/\s*([A-Z0-9]+)$", line)
        ...
        elif line in ("DOMESTIC", "INTERNATIONAL"):
            current["type"] = line
        elif line.startswith("ATB :") or line.startswith("ETB :"):
            current["atb"] = line.split(":", 1)[1].strip()
        elif line.startswith("ETD :") or line.startswith("ATD :"):
            current["etd"] = line.split(":", 1)[1].strip()
        elif "Open Stack :" in line:
            current["openstack"] = line.split(":", 1)[1].strip()
        elif "Closing Time Container" in line:
            current["closeC"] = line.split(":", 1)[1].strip()
        elif "PT" in line or "LTD" in line.upper() or "CO." in line.upper():
